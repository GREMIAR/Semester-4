...
namespace Heshed{
    partial class OurBlock{
        ...
        public int Search(int idRecordBook,string filename)
        {
            int end;
            int begin;
            int idHeshed=idRecordBook%4;
            using (var reader = File.Open(filename, FileMode.Open))
            {
                byte[] blockBinary = new byte[440];
                reader.Seek(4+idHeshed*4, SeekOrigin.Begin);
                reader.Read(blockBinary, 0, 8);
                byte[] intArr = new byte[4];
                Array.Copy(blockBinary,0,intArr,0,4);
                begin = BitConverter.ToInt32(intArr, 0);
                Array.Copy(blockBinary,4,intArr,0,4);
                end = BitConverter.ToInt32(intArr, 0);
                ...
                int numZapFound;
                do{
                    reader.Seek(begin, SeekOrigin.Begin);
